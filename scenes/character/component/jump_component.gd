...
@export var jump_height:float:
	get: return pow(jump_velocity,2)/(2*gravity_magnitude*fall_gravity_multiplier)
	set(val): jump_velocity = -sqrt(2*gravity_magnitude*fall_gravity_multiplier*val)
...
func update(body: CharacterBody2D, delta:float, is_jump_just_pressed: bool) -> void:
	...
	if body.is_on_floor():
		...
		body.velocity += gravity * delta * fall_gravity_multiplier
		if coyote_timer <= 0.0:
			is_jumping = false
			is_falling = true
